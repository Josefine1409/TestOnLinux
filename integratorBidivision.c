...
double recursiveIntegrateBidivision( double f(double),double a, double b, double delta, double epsilon, double f2, double f3, int nrec, int variableTransformationFormula){//wrapper calls F(f,t)
	...
	if (error < tolerance) {
		...
		double Q1 = recursiveIntegrateBidivision(f, a, a+(b-a)/2, delta/sqrt(2), epsilon, f1, f2, nrec + 1, variableTransformationFormula);
		double Q2 = recursiveIntegrateBidivision(f, a+(b-a)/2, b, delta/sqrt(2), epsilon, f3, f4, nrec + 1, variableTransformationFormula);
		return Q1 + Q2;
	}
}
